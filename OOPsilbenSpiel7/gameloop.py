        ...
        while True:
            ...
            for e in event.get():  # how to clear events?
                if e.type == QUIT:
                    ...
                    image_end, image_end_rect = self.info.default_font.render("GAME OVER", True, self.info.white)
                    image_end_rect.center = self.info.screen_copy.get_rect().center
                    self.info.default_font.render_to(self.info.screen_copy,image_end_rect,"GAME OVER",fgcolor=self.info.white)
            ...
            # AFTER GOING THROUGH THE EVENTS LIST
                ...
                # GUESSED WORDS WINDOW
                    ...
                    height_of_all = 0
                    rects_code_words = []
                    int_rect = self.info.default_font.render('99 ', True, self.info.white)[1]
                    spacing = int_rect.h
                    for i in range(len(self.info.woerter.input_code.split())): # the code?
                        if i < len(self.info.guessed_code_words):
                            ...
                            if self.info.guessed_code_words[i].name == self.info.woerter.input_code.split()[i]:
                                num_image, num_rect = self.info.default_font.render(f'{code_number_at_this_index}', True, self.info.white)
                            else:
                                num_image, num_rect = self.info.default_font.render(f'{opposite}', True,
                                                                          self.info.white)
                        else:
                            num_image, num_rect = self.info.default_font.render(f'{0 if i%2==0 else 1}', True,self.info.white)
                    ...
                    for i in range(len(self.info.guessed_code_words)): # combine w blit string?
                        ...
                        word_img, word_rect = font.render(word.name + " ", True, color)
                    ...
                    if " ".join([word.name for word in self.info.guessed_code_words]) == main.Main.codes[self.info.language-1]:
                        ...
                        if self.list_index_binary_click_words == indices_ones:
                            self.info.screen_copy.fill(self.info.black,Rect(0,height_of_all,self.info.screen_copy.get_rect().w,self.info.screen_copy.get_rect().h))
                            self.info.blit_string_words(f'RICHTIG',self.info.gold,height_of_all)
                            self.info.screen_transfer()
                            time.delay(10000)
                            return self.info.score
                        self.info.screen_transfer()
