...
class Woerter:
    ...
    def get_escape_game_text(self):
        with open('Woerterbuchspiel/escape_game_text', 'r') as file:
    ...
    def split_word_syls(self, word):  # german-specific #Angstschweiß
        ...
        def split_ks(
                ks):  # returns a tuple of an element to attach to previous part of hte word and one for the next part
            if len(ks) == 1:
                        ...
                        if any(el in word[i + 1:i + 3] for el in divide_after):
                            ...
                            for j in range(len(rest_word)):
                                if rest_word[j] in vowels:
                                    ...
                                    toappend += ks[
                                        0]  # try: attach (els from divide_after+next ks) to undividable vowel pairs
